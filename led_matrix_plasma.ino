...
Adafruit_NeoPixel pixels(40, 6, NEO_GRBW + NEO_KHZ800);
...
float colorStretch = 0.001;    // Higher numbers will produce tighter color bands. I like 0.11 . ok try .11 instead of .03

void setup() {
  pixels.begin();
  pixels.setBrightness(20); //please don't make it so bright!
...
void loop() {
  ...
  for( row=0; row<5; row++ ) {
    ...
    for( col=0; col<8; col++ ) {
      ...
      //pixels.setPixelColor(col + (8 * row), pixels.Color(color_4, 0, 0) );
